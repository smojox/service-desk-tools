...
import { TrendingUp, MessageCircle, Clock, User, Building, Ticket, ExternalLink, Loader2, Target } from "lucide-react"
import { CSITrackerItem } from "@/lib/models/CSITracker"

interface CSITrackerModalProps {
  item: CSITrackerItem | null
  isOpen: boolean
  onClose: () => void
  onUpdate: () => void
}

export default function CSITrackerModal({ item, isOpen, onClose, onUpdate }: CSITrackerModalProps) {
  ...
  const handleReassign = async (newAssigneeId: string, newAssigneeName: string) => {
    ...
    try {
      // Update the assignee
      const response = await fetch(`/api/csi-tracker/${item._id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ 
      ...
      if (response.ok) {
        ...
        const auditResponse = await fetch(`/api/csi-tracker/${item._id}/comments`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            content: `Ticket reassigned from ${item.assignedToName} to ${newAssigneeName}`,
            commentType: 'system'
          })
        })
  ...
  const handleAddComment = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`/api/csi-tracker/${item._id}/comments`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
                ...
                <div className="mt-2 space-y-2">
                  <div className="flex items-center gap-2">
                    <Progress 
                      ...
                      className="flex-1"
                    />
                    <span className="font-medium text-lg">{item.percentComplete}%</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <Input
                      type="number"
                    ...
                    <Button
                      size="sm"
                      onClick={() => handleProgressUpdate(item._id?.toString() || "", progressUpdate)}
                    >
                      Update
                    </Button>
                  </div>
